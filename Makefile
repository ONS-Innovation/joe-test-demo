...
security-scan:  ## Run security scan using Bandit.
	poetry run bandit -r joe_test_demo

.PHONY: test
test:  ## Run all the tests and check coverage.
	poetry run pytest -n auto --cov=joe_test_demo --cov-report term-missing --cov-fail-under=50 tests/

.PHONY: test-unit
test-unit: ## Run the unit tests and check coverage.
	poetry run pytest -n auto --cov=joe_test_demo --cov-report term-missing --cov-fail-under=50 tests/unit
